...
class Jadwal extends CI_Controller {
	...
	public function deletejadwal($kode)
	{
		...
		$this->getkod_model->delete('tbl_jadwal',$where);

		$this->session->set_flashdata('message', 'swal("Jadwal Berhasil Dihapus");');
		redirect(base_url('backend/jadwal'));
	}

	function editjadwal($kode){
        $where = array('kd_jadwal' => $kode);
        $data['jadwal'] = $this->getkod_model->edit_data('tbl_jadwal',$where)->result();
        $this->load->view(base_url('backend/jadwal/viewjadwal'),$data);
    }
	function updatejadwal(){
		$kode 			= $this->input->post('kd_jadwal');
		$bus 			= $this->input->post('kd_bus');
		$tujuan 		= $this->input->post('kd_tujuan');
		$asal 			= $this->input->post('kd_asal');
		$wilayah 		= $this->input->post('wilayah_jadwal');
		$berangkat		= $this->input->post('jam_berangkat_jadwal');
		$tiba			= $this->input->post('jam_tiba_jadwal');
		$harga			= $this->input->post('harga_jadwal');
	
		$data = array(
			'kd_bus' 				=> $bus,
			'kd_tujuan' 			=> $tujuan,
			'kd_asal' 				=> $asal,
			'wilayah_jadwal' 		=> $wilayah,
			'jam_berangkat_jadwal' 	=> $berangkat,
			'jam_tiba_jadwal' 		=> $tiba,
			'harga_jadwal'			=> $harga
		);
		...
		redirect(base_url('backend/jadwal/viewjadwal/'.$kode));
	}
}
